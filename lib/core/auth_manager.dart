...
class AuthManager {
  ...
  Future<void> saveOfflineLoginData(Map<String, dynamic> data) async {
    final dataJson = jsonEncode(data);
    await _secureStorage.write(key: StorageConstants.offlineLoginData, value: dataJson);
  }

  /// Get offline login data for guards
  Future<Map<String, dynamic>?> getOfflineLoginData() async {
    try {
  ...
  Future<void> saveAuthData({
    ...
    if (user.isGuard) {
      ...
      await saveOfflineLoginData(offlineData);
  ...
  /// Check if offline login is available for guards
  Future<bool> hasOfflineLoginData(String employeeId) async {
    try {
      final offlineData = await getOfflineLoginData();
  ...
  Future<UserModel?> getOfflineUserData(String employeeId) async {
    try {
      final hasOfflineData = await hasOfflineLoginData(employeeId);
  ...
  Future<void> clearOfflineLoginData() async {
  ...
  Future<void> clear() async {
    await Future.wait([
      ...
      clearLoginType(),
      clearSessionData(),
      // Note: We keep lastEmployeeId, offlineLoginData, and device data
      // unless explicitly cleared by user
    ]);
  }

  /// Complete clear including offline data (when user chooses to clear saved data)
  ...
  Future<void> clearOfflineData() async {
    final futures = StorageConstants.offlineKeys.map((key) => _clearKey(key));
    await Future.wait(futures);
  }
  ...
  Future<Map<String, dynamic>> getAuthInfo() async {
    ...
    return {
      ...
      'hasOfflineData': await hasOfflineLoginData(lastEmployeeId ?? ''),
    };
  }
